...
if __name__ == "__main__":
    ...
    while True:
        ...
        choice = main_menu(execution_id=execution_id)
        lr.log_start(function_name="main_menu", input_details={"choice": choice})

        while True:
            if choice == 1:
                choice = user_menu(execution_id=execution_id)
                lr.log_start(function_name="user_menu", input_details={"choice": choice})
                if choice == 1:
                    user_id, user_name, user_email, user_age, user_gender = user_input()
                    lr.log_start(function_name="user_input",
                             input_details={"user_id": user_id, "user_name": user_name, "user_email": user_email,
                                            "user_age": user_age, "user_gender": user_gender})
                    user = User(user_id, user_name, user_email, user_age, user_gender)
                    lr.log_start(function_name="User.__init__",
                             input_details={"user_id": user_id, "user_name": user_name, "user_email": user_email,
                                            "user_age": user_age, "user_gender": user_gender})
                    result = UserOperation().is_valid_user(user)
                    lr.log_start(function_name="UserOperation().is_valid_user", input_details={"user": user})
                    if result == "success":
                        result = UserStorage().save_user(user)
                        if result == True:
